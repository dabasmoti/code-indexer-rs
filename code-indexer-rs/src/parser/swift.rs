...
impl super::LanguageParser for SwiftParser {
    ...
    fn parse_symbols(&self, source: &[u8], file_path: &Path) -> Vec<Symbol> {
        ...
        let query_src = r#"
            ...
            (struct_declaration name: (type_identifier) @name) @struc
            (enum_declaration name: (type_identifier) @name) @enu
            (protocol_declaration name: (type_identifier) @name) @proto
        "#;
    ...
    fn parse_imports(&self, source: &[u8], file_path: &Path) -> Vec<Import> {
        ...
        let query_src = r#"(import_declaration (identifier) @path) @imp"#;
        let query = match Query::new(&self.language, query_src) {
            Ok(q) => q,
            Err(_) => return vec![],
        };
    ...
    fn chunk_file(&self, source: &[u8], file_path: &Path) -> Vec<Chunk> {
        ...
        let tree = match parser.parse(source, None) {
            Some(t) => t,
            None => return chunker::chunk_by_lines(source_str, file_path, Language::Swift, &[]),
        };
        ...
        chunker::chunk_by_lines(source_str, file_path, Language::Swift, &boundaries)
    }
}
