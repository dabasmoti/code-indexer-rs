...
impl super::LanguageParser for GoParser {
    ...
    fn parse_symbols(&self, source: &[u8], file_path: &Path) -> Vec<Symbol> {
        ...
        let query_src = r#"
            ...
            (type_declaration (type_spec name: (type_identifier) @name)) @typ
        "#;
    ...
    fn chunk_file(&self, source: &[u8], file_path: &Path) -> Vec<Chunk> {
        ...
        let tree = match parser.parse(source, None) {
            Some(t) => t,
            None => return chunker::chunk_by_lines(source_str, file_path, Language::Go, &[]),
        };
        ...
        chunker::chunk_by_lines(source_str, file_path, Language::Go, &boundaries)
    }
}
