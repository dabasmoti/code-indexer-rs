...
impl super::LanguageParser for PythonParser {
    ...
    fn parse_imports(&self, source: &[u8], file_path: &Path) -> Vec<Import> {
        ...
        let query_src = r#"
            (import_statement name: (dotted_name) @path) @imp
            (import_from_statement module_name: (dotted_name) @path) @imp
        "#;
        let query = match Query::new(&self.language, query_src) {
            Ok(q) => q,
            Err(_) => return vec![],
    ...
    fn chunk_file(&self, source: &[u8], file_path: &Path) -> Vec<Chunk> {
        ...
        let tree = match parser.parse(source, None) {
            Some(t) => t,
            None => return chunker::chunk_by_lines(source_str, file_path, Language::Python, &[]),
        };
        ...
        chunker::chunk_by_lines(source_str, file_path, Language::Python, &boundaries)
    }
}
